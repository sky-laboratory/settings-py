# =============================================================================
# CollabOps settings-py: Django Stack - python.mk
# =============================================================================
...
.PHONY: check-venv sync format lint test test-unit test-integration test-cov \
        local local-it build-it clean clean-all run migrate makemigrations \
        shell collectstatic createsuperuser help
...
RUFF           := $(UV) run ruff
MYPY           := $(UV) run mypy
DJANGO_MANAGE  := $(UV) run python $(MANAGE_PY)
...
help: ## Show this help message
	@echo "$(COLOR_BLUE)CollabOps Django Stack - Available Commands$(COLOR_RESET)"
	@echo ""
...
run: check-venv ## Run Django development server (0.0.0.0:8000)
	@echo "$(COLOR_BLUE)[INFO] Starting Django server...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) runserver $(APP_HOST):$(APP_PORT)

run-prod: check-venv ## Run Django with gunicorn (production mode)
	@echo "$(COLOR_BLUE)[INFO] Starting Django with gunicorn...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=config.settings.prod \
		$(UV) run gunicorn config.wsgi:application \
		-w $(APP_WORKERS) \
		-b $(APP_HOST):$(APP_PORT)

migrate: check-venv ## Run Django migrations
	@echo "$(COLOR_BLUE)[INFO] Running migrations...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) migrate
	@echo "$(COLOR_GREEN)[OK] Migrations applied$(COLOR_RESET)"

makemigrations: check-venv ## Create new Django migrations
	@echo "$(COLOR_BLUE)[INFO] Creating migrations...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) makemigrations
	@echo "$(COLOR_GREEN)[OK] Migrations created$(COLOR_RESET)"

shell: check-venv ## Open Django shell (shell_plus if available)
	@echo "$(COLOR_BLUE)[INFO] Opening Django shell...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) shell_plus 2>/dev/null || \
		DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) shell

collectstatic: check-venv ## Collect static files
	@echo "$(COLOR_BLUE)[INFO] Collecting static files...$(COLOR_RESET)"
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) collectstatic --noinput
	@echo "$(COLOR_GREEN)[OK] Static files collected$(COLOR_RESET)"

createsuperuser: check-venv ## Create Django superuser
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) createsuperuser

dbshell: check-venv ## Open database shell
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
		$(DJANGO_MANAGE) dbshell

showmigrations: check-venv ## Show migration status
	DJANGO_SETTINGS_MODULE=$(DJANGO_SETTINGS) \
...
local-migrate: local ## Run migrations in local Docker environment
	@echo "$(COLOR_BLUE)[INFO] Running migrations in Docker...$(COLOR_RESET)"
	$(DOCKER_COMPOSE) -f $(LOCAL_COMPOSE_FILE) exec app python manage.py migrate
	@echo "$(COLOR_GREEN)[OK] Migrations applied$(COLOR_RESET)"
...
clean: ## Clean temporary files and caches
	...
	rm -rf dist/ build/ staticfiles/ 2>/dev/null || true
